...
const SeatLayout = () => {
  ...
  const {axios, getToken, user, setPaymentUrl} = useAppContext();
    ...
    <div className='flex flex-col md:flex-row px-6 md:px-16 lg:px-40 py-30 md:pt-50'>
      ...
      <div className='w-60 bg-primary/10 border border-primary/20 rounded-lg py-10 h-max md:sticky md:top-30'>
        ...
        <div className='mt-5 space-y-1'>
          {show.dateTime[date].map((item)=>{
            return (
              <div key={item.showId} onClick={()=> setSelectedTime(item)} className={`flex items-center gap-2 px-6 py-2 w-max rounded-r-md cursor-pointer transition ${selectedTime?.time === item.time ? "bg-primary text-white" : "hover:bg-primary/20"}`}>
      ...
      <div className='relative flex-1 flex flex-col items-center max-md:mt-16'>
          ...
          <div className='flex flex-col items-center mt-10 text-xs text-gray-300'>
            <div className='grid frid-cols-2 md:grid-cols-1 gap-8 md:gap-2 mb-6'>
